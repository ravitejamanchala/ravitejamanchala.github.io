...
export const useGetDataset = (stale) => {
  const reducer = (state, action) => {
    switch (action.type) {
      case FetchState.FETCH_INIT:
        return { ...state, isLoading: true, isError: false };
  ...
  console.log('Database ID:', Server.databaseID);
  console.log('Dataset ID:', Server.datasetID);
  useEffect(() => {
    let didCancel = false;
    const getImages = async () => {
      dispatch({ type: FetchState.FETCH_INIT });
      try {
        const data = await api.listDocuments(Server.databaseID, Server.datasetID);
        if (!didCancel) {
          dispatch({ type: FetchState.FETCH_SUCCESS, payload: data.documents });
        }
      } catch (e) {
        console.error('Error getting dataset:', e);
        if (!didCancel) {
          dispatch({ type: FetchState.FETCH_FAILURE });
        }
      }
